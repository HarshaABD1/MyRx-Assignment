...
departure = ["8:30", "8:25", "8:35","8:40", "8:50"]

def convert_mins(arrival, departure):
    arr_in_minutes = []
    dep_in_minutes = []
    for i in range(len(arrival)):
        ...
        arr_in_minutes.append(arrival_hours * 60 + arrival_mins)
        dep_in_minutes.append(departure_hours * 60 + departure_mins)
    return arr_in_minutes, dep_in_minutes

def max_platforms_needed(arrival, departure ):
    print("arrival :" , arrival )
    print("arrival type :", type(arrival))
    if len(arrival) == 0 or len(departure) == 0:
        return 0
    elif len(arrival) != len(departure):
        return 0 
    else:
        a_in_minutes, d_in_minutes = convert_mins(arrival, departure)
        max_platforms = 1  
...
result = max_platforms_needed(arrival, departure)
print(result)